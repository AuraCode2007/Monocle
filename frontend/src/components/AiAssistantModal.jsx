...
import { Bot, X, Sparkles, ArrowRight, CornerDownLeft, ShieldCheck } from 'lucide-react';
...
export default function AiAssistantModal({ isOpen, onClose }) {
  ...
  const { getTrains, isOptimized, toggleOptimize, setActiveTab, setFilterDept } = useRailwayStore();
  ...
  const processQuery = (rawQuery) => {
    ...
    if (q.includes('rajdhani') || q.includes('vande bharat') || q.includes('train')) {
      const priorityTrains = trains.filter((train) => train.type === 'RAJDHANI' || train.type === 'VANDE_BHARAT');
      const trainNames = priorityTrains.length
        ? priorityTrains.map((train) => `${train.number} ${train.name}`).join(', ')
        : 'No Rajdhani or Vande Bharat service is loaded on this corridor.';
      const msg = isOptimized
        ? `No active conflicts remain after optimization. Priority services in the current corridor plan: ${trainNames}.`
        : `The manual plan requires review against ${trainNames}. Open the time-distance chart to inspect each train path against the requested blocks.`;
      setResponse({
        type: 'TRAIN_QUERY',
        title: isOptimized ? 'Priority train conflict check' : 'Priority train conflict review',
        text: msg,
      ...
      setResponse({
        ...
        text: msg,
        actionText: isOptimized ? 'View Form T/348M PTW' : 'Run optimization',
        action: () => { if (!isOptimized) toggleOptimize(); setActiveTab('PTW'); onClose(); }
      });
    } else if (q.includes('calendar') || q.includes('rolling block') || q.includes('26-week')) {
      setResponse({
        type: 'CALENDAR_QUERY',
        title: 'Rolling block capacity',
        text: 'The rolling calendar contains recurring maintenance windows, possession capacity, and department coordination requirements for the next 26 weeks.',
        actionText: 'Open rolling calendar',
  ...
  const handleSuggestedQuery = (prompt) => {
    setQuery(prompt);
    processQuery(prompt);
  };
      ...
      <div className="w-full max-w-2xl glass-card rounded-2xl border border-slate-700 shadow-2xl p-5 flex flex-col overflow-hidden">
        ...
        <form onSubmit={handleSearch} className="flex items-center gap-3 pb-4 border-b border-slate-800">
          ...
          <input
            ...
            autoFocus
          />
          <button type="submit" aria-label="Submit operations query" title="Submit query" className="p-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold cursor-pointer transition-all">
        ...
        {response && (
          <div className="py-4 space-y-3 animate-fadeIn">
            <div className="flex items-center justify-between gap-3">
              <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500">{ui.response}</div>
              <button type="button" onClick={() => { setResponse(null); setQuery(''); }} className="text-[11px] font-semibold text-slate-400 hover:text-white">{ui.newQuery}</button>
            </div>
        ...
        <div className="pt-3 border-t border-slate-800/80 text-[11px] text-slate-500 flex justify-between items-center">
          <span>Press <kbd className="px-1.5 py-0.5 rounded bg-slate-800 text-slate-300 font-mono">Ctrl K</kbd> to open commands</span>
          <span>{ui.operationsSearch}</span>
        </div>
      </div>
    </div>
  );
